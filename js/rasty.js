...
function submitFormulario( formId ){
	...
	if( resp ){
		if((formId == 'pista_form')&&($("#mp3").val()=='')){
			if(confirm('No subió ningún DEMO (Le recuerdo que debe presionar "Subir demo" antes de presionar "confirmar") ¿Continuar?')){
				$( "#" + formId ).submit();	
			}
		}	
		if((formId == 'artista_form')&&($("#imagen").val()=='')){
			if(confirm('No subió ninguna imagen (Le recuerdo que debe presionar "Subir imagen" antes de presionar "confirmar") ¿Continuar?')){
				$( "#" + formId ).submit();	
			}
		}	
		else{
