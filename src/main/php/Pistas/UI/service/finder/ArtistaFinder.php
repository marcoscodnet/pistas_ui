<?php
namespace Pistas\UI\service\finder;
...
/**
 * 
 * Finder para artistas.
 * 
 * @author Marcos
 * @since 16/03/2018
 */
class ArtistaFinder implements  IAutocompleteFinder {
	...
	public function findEntitiesByText( $text, $parent=null ){
		...
		$uiCriteria = new UIArtistaCriteria();
		$uiCriteria->setNombre( $text );
		$uiCriteria->setRowPerPage( 10 );
		return UIServiceFactory::getUIArtistaService()->getList($uiCriteria);	
	}
	...
	function findEntityByCode( $code, $parent=null ){
		...
		return UIServiceFactory::getUIArtistaService()->get( $code );
	}
